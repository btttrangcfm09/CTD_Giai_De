...
#include <stdlib.h>
#ifdef _WIN32
  #include <conio.h>  // getch() cho Windows
#else
  #define getch() getchar()  // getchar() cho Linux/Mac
#endif
...
//      noecho();
    ...
    case OP_PW: // đề 2020 bài 1: phép mũ nguyên
      t--;
      if (checkStack()) {
        WORD baseVal = stack[t];
        WORD expVal = stack[t+1];
        if (expVal < 0) {
          // Không hỗ trợ mũ âm trong số nguyên
          stack[t] = 0;
        } else {
          WORD result = 1;
          WORD b = baseVal;
